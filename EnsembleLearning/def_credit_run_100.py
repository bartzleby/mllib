...
def main():
  ...
  H = AdaBoost(100, train[:,0:-1], attribute_dict, train[:,-1], dtype=dtype)
  with open('./pickle/def_credit_run_adaboost_100.pkl', 'wb') as file: 
  ...
  trees = BaggedTrees(2000, 100, train[:,0:-1], attribute_dict, train[:,-1], dtype=dtype)
  forest = RandomForest(2000, 100, train[:,0:-1], attribute_dict, train[:,-1], dtype=dtype, NumRandAttr=5)

  with open('./pickle/def_credit_run_100.pkl', 'wb') as file: 
    pickle.dump(attribute_dict, file) 
    pickle.dump(train, file)
    pickle.dump(test, file)
    pickle.dump(H, file)
